...
response = event.map(lambda x : replier.UnstructuredReplyToBroker('{ "value" :' + str(x+10) +' }'))
response2 = event.map(lambda x: replier.SemistructuredReplyToBroker(x+10, '{"value" : %%TOREPLACE%% }'))
response3 = event.map(lambda x : replier.ReplyToBroker(x+10))
event.pprint()
response.pprint()
response2.pprint()
response3.pprint()
